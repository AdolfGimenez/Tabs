...
456817,456818,456820,456821,456822,456823,456824) ;          --Mensaje retornado por SIFEN (se usa para verificar el motivo de rechazo)
...
select * FROM sirius.sifen.EMPRESASUCURSALCAJA E WHERE E.IDCAJA = 1;
...
SELECT * FROM sirius.sifen.EMPRESASUCURSALCAJA E;
SELECT * FROM SIRIUS.BASE;

SELECT T.NRODELDOCUMENTO, T.* FROM SIFEN.DOCUMENTOELECTRONICO D
INNER JOIN SIFEN.TIMBRADO T ON D.IDEMPRESA=T.IDEMPRESA AND D.IDDE=T.IDDE
WHERE T.NRODELDOCUMENTO IN (359748, 359724, 359749, 359723, 359745, 359721, 359722, 359741, 359735, 359710, 359746, 359716, 359702, 359732, 359718, 359743, 359708, 359734, 359720, 359739, 359709, 359729, 359714, 359744, 359706, 359733, 359707, 359719, 359728, 359742, 359704, 359717, 359740, 359731, 359730, 359715, 359705, 359738, 359727, 359750, 359713, 359703, 359737, 359726, 359712, 359701, 359736, 359725, 359711, 359699)
--AND T.IDCAJA=13;

SELECT D.* FROM SIFEN.DOCUMENTOELECTRONICO D WHERE idde IN (1141604, 1141621, 1141590, 1141606, 1141618, 1141595, 1141614, 1141620, 1141591, 1141585, 1141598, 1141603, 1141587, 1141605, 1141624, 1141610, 1141623, 1141578, 1141582, 1141588, 1141600, 1141599, 1141615, 1141596, 1141608, 1141625, 1141619, 1141607, 1141622, 1141594, 1141609, 1141586, 1141611, 1141616, 1141583, 1141601, 1141584, 1141579, 1141592, 1141580, 1141626, 1141577, 1141589, 1141602, 1141581, 1141597, 1141612, 1141593, 1141613, 1141617);
INNER JOIN SIFEN.TIMBRADO T ON D.IDEMPRESA=T.IDEMPRESA AND D.IDDE=T.IDDE
...
SELECT * from base.sis_param_value;

SELECT DISTINCT idde FROM sirius.sifen.LOTEDETALLEEVENTO L WHERE L.IDTIPOEVENTO=19 AND L.IDTIPOESTADOEVENTO=4;

SELECT * FROM sirius.sifen.GENERALDERECEPTOR G
         WHERE idde IN (724259, 781815, 674048, 589410, 780187, 574252, 473348, 601710, 695857, 589659, 674001, 577871, 643032, 674055, 660995, 625212, 654312, 557619, 576078, 645325, 590921, 795086, 479301, 458652, 799543, 631606, 697510, 603065, 654945, 700161, 628555, 472251, 601914, 721829, 465919, 547607, 615550, 480351, 803444, 462062, 605668, 444988, 484565, 549055, 464648, 447814, 492828, 657338, 621422, 847662, 725115, 659169, 705160, 550482, 594165, 701649, 479933, 621312, 571755, 547830, 589707, 721758, 480766, 726921, 465648, 458534, 635935, 660522, 479350, 665305, 589324, 659090, 590710, 696377, 481880, 490817, 632685, 471115, 520751, 661383, 577097, 722986, 630441, 446727, 724255, 629186, 785774, 612952, 650620, 630908, 478996, 798094, 579952, 546555, 661427, 608442, 626536, 639722, 664903, 603180, 470286, 661161, 489160, 547290, 770037, 488603, 465920, 580372, 596047, 466970, 603615, 722992, 483611, 662511, 584192, 448358, 548479, 601670, 626881, 595843, 590084, 447009, 546518, 579475, 593101, 448182, 445903, 545852, 572315, 455996, 480261, 663506, 656568, 584710, 627880, 634334, 488403, 466094, 476221, 748555, 645490, 784082, 663793, 781480, 601042, 545373, 574438, 466061, 660336, 628125, 682710, 621459, 543414, 548316, 703266, 659385, 655244, 650822, 580528, 492916, 701069, 750349, 721771, 699720, 578102, 593347, 726281, 579077, 574546, 674397, 621338, 684194, 555826, 700648, 546978, 721753, 749137, 654817, 633886, 613939, 801091, 606322, 658249, 611861, 614054, 814323, 465231, 724250, 510214, 657978, 574305, 485855, 602384, 546313, 574520, 726277, 474046, 574079, 721767, 573053, 695204, 689099, 713016, 680968, 647764, 687979, 591956, 646672, 588063, 446174, 570667, 627797, 480657, 621425, 659759, 547535, 781486, 747443, 584746, 548954, 542359, 612711, 487908, 596056, 602791, 650870, 573653, 647302, 556167, 485031, 721750, 585644, 627464, 460036, 586413, 629398, 588360, 691910, 607381, 628175, 588997, 632813, 470027, 626828, 484764, 787549, 469826, 481035, 486813, 468863, 547223, 601022, 608793, 659426, 549657, 621404, 589849, 464600, 634535, 673785, 467175, 467778, 604671, 520591, 670828, 453318, 606694, 628318, 603525, 471301, 658294, 658802, 653733, 479449, 595396, 570302, 447279, 551815, 655750, 653563, 618953, 640711, 658378, 587933, 626979, 621160, 480604, 594296, 781495, 587622, 667341, 473846, 657783, 659525, 782783, 625498, 682196, 624120, 465558, 471878, 543430, 726263, 573670, 550361, 447648, 588410, 464747, 588511, 621360, 798383, 792625, 481675, 576693, 485027, 570871, 466330, 457554, 444199, 444210, 486696, 725175, 647835, 446219, 764246, 522430, 663352, 592415, 772510, 456500, 601901, 652963, 464295, 589815, 605468, 445049, 472130, 781490, 662252, 574653, 547491, 628283, 639916, 578966, 553251, 674121, 587551, 621458, 585812, 669471, 447410, 651686, 631854, 447327, 657712, 767551, 649056, 615385);

/*[3:08 p.m., 7/2/2025] Silvana : hay una tabla que se condicionoperacion
[3:09 p.m., 7/2/2025] Silvana : ahi se guarda el documento con su idde y el idtipocondicionoperacion
[3:09 p.m., 7/2/2025] Silvana : y hay 2 tablas más tambien que se llaman condicionoperacionpagocontado y condicionoperacionpagocredito
[3:10 p.m., 7/2/2025] Silvana : dependiendo del tipo condicion operacion el documento se va en una tabla u otra*/

SELECT * FROM sirius.sifen.CONDICIONOPERACION C WHERE C.IDDE  IN (587613, 587616, 601842, 732001);


SELECT * FROM sirius.sifen.TIPOCONDICIONOPERACION T;
SELECT * FROM sirius.sifen.CONDICIONOPERACIONPAGOCONTADO C WHERE C.IDDE=645742 AND C.IDCONDICIONOPERACIONPAGO=1
    C.IDDE  IN (587613, 587616, 601842, 645742);
SELECT * FROM sirius.sifen.CONDICIONOPERACIONPAGOCREDITO C WHERE C.IDDE  IN (587613, 587616, 601842, 645742);

INSERT INTO SIFEN.CONDICIONOPERACIONPAGOCREDITO (IDEMPRESA, IDDE, IDTIPOCONDICIONOPERACIONCREDITO, PLAZOCREDITO, CANTIDADCUOTA, MONTOENTREGAINICIAL)
VALUES (2, 587613, 2, NULL, 1, 0.0000);
INSERT INTO SIFEN.CONDICIONOPERACIONPAGOCREDITO (IDEMPRESA, IDDE, IDTIPOCONDICIONOPERACIONCREDITO, PLAZOCREDITO, CANTIDADCUOTA, MONTOENTREGAINICIAL)
VALUES (2, 587616, 2, NULL, 1, 0.0000);
INSERT INTO SIFEN.CONDICIONOPERACIONPAGOCREDITO (IDEMPRESA, IDDE, IDTIPOCONDICIONOPERACIONCREDITO, PLAZOCREDITO, CANTIDADCUOTA, MONTOENTREGAINICIAL)
VALUES (2, 601842, 2, NULL, 1, 0.0000);

SELECT * FROM sirius.sifen.TIPOPAGO T;

SELECT * FROM sirius.sifen.DOCUMENTOELECTRONICO D WHERE D.IDDE=266729;
SELECT * FROM INTEGRACION.DTO D WHERE D.IDDTO=5200122;
select * FROM sirius.sifen.ITEMOPERACION I WHERE I.IDDE=985754;
