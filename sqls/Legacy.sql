...
SELECT * FROM visa.ACFIRECD a; --TABLA EMISOR COLA
SELECT
    DATA_QUEUE_LIBRARY, DATA_QUEUE_NAME , MAXIMUM_MESSAGE_LENGTH, CURRENT_MESSAGES
FROM QSYS2.DATA_QUEUE_INFO WHERE DATA_QUEUE_NAME LIKE '%ACFIREC%';
--SELECT CURSOR
select * from libdebito.drconbep where lerrnb in
(SELECT RRN_EXT FROM VISA.NRT_RRN_EX where NRTLOGFECH='2025-07-08'
EXCEPT SELECT LERRNB FROM LIBDEBITO.DRCONBEP where
LFECTR = '20250708'  AND LHORTR>='110658'
UNION
SELECT LERRNB FROM LIBDEBITO.DRCONBEP where  LFECTR = '20250708' AND LHORTR>='110658'
EXCEPT SELECT RRN_EXT FROM VISA.NRT_RRN_EX where
NRTLOGFECH='2025-07-08');

--PRUEBA MANUAL
SBMJOB CMD(CALL PGM(VISA/ACFIGETLOG)) JOBQ(QAUTORIZA) USER(VISAUSR)
