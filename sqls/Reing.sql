...
SELECT * FROM GXTRJDTA.tdgciu WHERE DEPIDDEPT = 17;
...
A.SETRRNBEP  NRO_REFERENCIA, A.SETFECPRO  FECHA_PROCESO, A.SETIMPCOD  COD_IMPUESTO,
A.SETCODMCC  MCC, A.SETCICLIE  DOC_CLIENTE, A.SETNOMPLA  CLIENTE, A.SETRZNPRV  PROVEEDOR,
A.SETMVDSPA  PAIS_ORIGEN, A.SETMARCA  MARCA, A.SETTIPTAR  TIPO_TARJETA, A.SETCODEM  EMISOR,
A.SETMONTO  IMPORTE_ORIGEN, A.SETLIMPAU  IMPORTE_LOCAL, A.SETIMPIVA  IMPORTE_IVA, A.SETIMPREN  IMPORTE_RENTA,
B.RETESTPRO  RESPUESTA_SET, B.RETTXTPRO,
B.RETNROCMP  NRO_COMPROBANTE,
A.SETFECAC  FECHA_ACT,
A.SETUSUAC  USUARIO
--DISTINCT A.SETRETSTA, COUNT(*)
FROM GXFINDTA.TCLSET A
         LEFT JOIN GXFINDTA.TCLRET B
                   ON A.SETIDRET = B.RETIDRET
WHERE A.SETFECPRO BETWEEN '20240101' AND '20240531';
--GROUP BY A.SETRETSTA ;
...
ON a.SETidRet = b.RETIdRet
WHERE a.SETFecPro = '20250401'; --- trae datos
...
---SUCURSALES ASIGNADAS- PORTAL
SELECT (SELECT T.COCOMER
        ...
        LIMIT 1)                     AS COCOMER,
       (SELECT C.COMADRE FROM GXWEB.COMAEAF C WHERE C.COCOMER = (SELECT T.COCOMER
FROM GXFINDTA.TCOCNA T WHERE T.CLICLICOD = A.CLICLICOD AND T.SUCSUCCOD = A.SUCSUCCOD   AND SUBSTR(T.COCOMER, 0, 3) NOT IN ('45')
ORDER BY SUBSTR(T.COCOMER, 0, 3)
LIMIT 1)) AS COMADRE, A.*, B.*
FROM GXFINDTA.TCMSUC A
         INNER JOIN GXFINDTA.TCMCLI B ON A.CLICLICOD = B.CLICLICOD
WHERE A.SUCESTADO NOT IN ('B')
AND A.CLICLICOD = '10836'
 AND A.SUCSUCCOD = '17';