...
FROM FACTURACIONBEPSA.TCLMOV_TMP_COMERCIOS_202501_FACTCOMERCIOS AS "c"
WHERE "c"."movcodcli" = '11728'
--  AND "c"."movcodsuc" = '4'
...
SELECT F.FACVENTAID,F2.FACVENTAFECHA, F2.FACVENTACOMPROBANTESET, F2.FACVENTADESC, F2.FACVENTACLIRUC, F2.CLIENTEID, F2.SUCURSALID,
...
'C-000', 2025, 01, 'FactComercios', 'PYG',
...
SELECT * FROM FACTURACIONBEPSA.TIPOASIENTOCONTABLEDET T WHERE T.TIPOCOMPROBANTEID='FAECA'
AND T.TIPOASIENTOCONTABLEID='COMIPESA'
...
----------------------------------------------------------------------------
--para borrar facturas, ELIMINACION MASIVA DE FACTURAS
DELETE
FROM FACTURACIONBEPSA.FACVENTADET VD
WHERE EXISTS (SELECT 1
              FROM FACTURACIONBEPSA.FACVENTA F
              WHERE F.FACVENTAFECHA = '2025-02-05'
                AND F.TARGETFACTID = 'FACTALQPOS'
                AND F.STATUSID = 'preformulario'
...
select * from TRUSTED_ZONE.GXFINDTA_TCLMOV GT;

SELECT * from asientos.datos.GXBDBPS_COMAEAF GC WHERE GC.CORUCN='2155702-0';

SELECT * FROM asientos.facturacionbepsa.FACVENTACUOTA F
         WHERE F.facventaid IN (860833, 864085, 864314, 865140, 867207, 867274, 867386, 869964, 873257, 875328, 876460, 876598);
--RETENCIONES
--para no contribuyentes:
select * from facturacionbepsa.fn_get_info_retenciones_set();
--y este es para no contribuyentes:
SELECT * FROM facturacionbepsa.fn_get_info_retenciones_tesaka();
--CABECERA RETENCIONES
SELECT * FROM facturacionbepsa.fn_get_info_retenciones_tesaka();
SELECT
        f.facventaid AS facventaid, -- id de factura para poder relacionar internamente, NO entra en el JSON
        f.monedaid::varchar AS retencion_moneda,
        f.facventafecha::date AS retencion_fecha, -- ?
        true::boolean AS retencion_retencionRenta, -- ?
        /*v_conceptoRenta::varchar AS retencion_conceptoRenta,*/ -- ?
        0.90909::numeric AS retencion_ivaPorcentaje5, -- ? se utiliza el mismo que en fn_get_info_retenciones_set (retencion_ivaporcentaje).
        0.90909::numeric AS retencion_ivaPorcentaje10, -- ? se utiliza el mismo que en fn_get_info_retenciones_set (retencion_ivaporcentaje).
        0::numeric AS retencion_rentaCabezasBase, -- ?
        0::numeric AS retencion_rentaCabezasCantidad, -- ?
        0::numeric AS retencion_rentaToneladasBase, -- ?
        0::numeric AS retencion_rentaToneladasCantidad, -- ?
        1::numeric AS retencion_rentaPorcentaje, -- se utiliza el mismo que en fn_get_info_retenciones_set. OBS: Obtener por paramétricas
        true::boolean AS retencion_retencionIva, -- ? se utiliza el mismo que en fn_get_info_retenciones_set (retencion_retencioniva).
        /*v_conceptoIva::varchar AS retencion_conceptoIva, -- ?*/
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN c.clienteruc ELSE NULL END)::varchar AS informado_ruc, -- obligatorio para contribuyente
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN c.clientedv ELSE NULL END)::varchar AS informado_dv, -- obligatorio para contribuyente
        c.clienterazonsocial::varchar AS informado_nombre,
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN NULL ELSE c.clienteruc END)::varchar AS informado_identificacion, -- nro doc, obligatorio para NO contribuyentes
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN 'CONTRIBUYENTE' ELSE 'NO_CONTRIBUYENTE' END)::varchar AS informado_situacion,
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN NULL ELSE 'CEDULA' END)::varchar AS informado_tipoIdentificacion, -- obligatorio para NO contribuyentes
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN NULL ELSE c.clienteemail END)::varchar AS informado_correoElectronico,  -- obligatorio para NO contribuyentes
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN NULL ELSE c.clientedomicilio END)::varchar AS informado_direccion, -- obligatorio para NO contribuyentes
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN NULL ELSE c.clientephone END)::varchar AS informado_telefono, -- obligatorio para NO contribuyentes
        'PY'::varchar AS informado_pais, -- OBS: Obtener por paramétricas
        null::varchar AS informado_nombreFantasia, -- aparece en JSON pero no tiene descripción en documentación
        (CASE WHEN c.clientetipocontrib = 'CONTE' THEN c.clientedomicilio ELSE NULL END)::varchar AS informado_domicilio, -- obligatorio para contribuyentes
        false::boolean AS informado_tieneRepresentante, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        false::boolean AS informado_tieneBeneficiario, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        null::varchar AS informado_representante_tipoIdentificacion, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        null::varchar AS informado_representante_identificacion, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        null::varchar AS informado_representante_nombre, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        null::varchar AS informado_beneficiario_tipoIdentificacion, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        null::varchar AS informado_beneficiario_identificacion, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        null::varchar AS informado_beneficiario_nombre, -- solo se utiliza en no_residente. No tenemos no_residentes en BD
        f.facventacomprobanteset::varchar AS transaccion_numeroComprobanteVenta,
        (CASE WHEN f.facventacondicion = 'CRDTO' THEN 'CREDITO' ELSE 'CONTADO' END)::varchar AS transaccion_condicionCompra,
        (CASE WHEN f.facventacondicion = 'CRDTO' THEN (SELECT count(1) FROM facturacionbepsa.facventacuota fvc WHERE fvc.facventaid = f.facventaid) ELSE 0 END)::integer AS transaccion_cuotas,
        1::integer AS transaccion_tipoComprobante, -- ?  (1 = Factura) en Documentación. OBS: Agregar por paramétrica
        f.facventafecha::date AS transaccion_fecha,
        f.timbradoid::varchar AS transaccion_numeroTimbrado,
        current_date::date AS atributos_fechaCreacion,
        TO_CHAR(current_timestamp, 'YYYY-MM-DD HH:mm:SS')::varchar AS atributos_fechaHoraCreacion
    FROM
        facturacionbepsa.facventa f
        INNER JOIN facturacionbepsa.facventadet fvd ON f.facventaid = fvd.facventaid
        INNER JOIN facturacionbepsa.cliente c ON c.tipoclienteid = f.tipoclienteid AND c.clienteid = f.clienteid
        LEFT JOIN facturacionbepsa.facventartset fvs ON f.facventaid = fvs.facventaid
        AND fvs.facventartsetprocesamientocorr = true
    WHERE
        f.ejercicioid = 2025
        AND f.periodoid = 01
        AND f.tipocomprobanteid = 'FAECA'
        AND f.timbradoid = '16410772'
        AND f.establecimientoid = '001'
        AND f.puntoexpedicionid = '002'
        AND f.timbradocomprobcajaid = 'C-000'
        AND f.targetfactid = 'FactComercios'
        AND f.monedaid = 'PYG'
        AND f.tipoclienteid = 'Comercio'
        AND f.facventacondicion = 'CNTDO'
        AND c.tipoclienteid = 'Comercio'
        AND c.clientetipocontrib = 'CONTE' -- nueva condición para discriminar por tipo contribuyente
        AND f.statusid IN ('DTE_APROBADO')
        AND fvd.servprestid IN (
        SELECT * FROM facturacionbepsa.fn_obtener_parmvalue ('desc_serv_prest', 'C'))
        AND (f.facventaretensetmonto + f.facventaretensetiva) > 0
        AND fvs.facventaid IS NULL --COMENTAR SI YA SE PROCESO
        --AND f.FACVENTAID  IN (SELECT FACVENTAID FROM PUBLIC.FACVENTARTSETBK F2)
    ORDER BY
        f.facventaid;

--DETALLE
SELECT * FROM facturacionbepsa.fn_get_facventadet_retencion(911337);

SELECT FD.FACVENTAID,
       FD.FACVENTADETCANT          AS CANTIDAD,
       S.TIPOIVAID::INTEGER        AS TASAAPLICA,
       FD.FACVENTADETPRECIOML      AS PRECIOUNITARIO,
       FD.FACVENTADETSERVPRESTDESC AS DESCRIPCION
FROM FACTURACIONBEPSA.FACVENTADET FD
         INNER JOIN FACTURACIONBEPSA.SERVPREST S ON S.SERVPRESTID = FD.SERVPRESTID
WHERE FD.FACVENTAID IN (SELECT F.FACVENTAID
                        FROM FACTURACIONBEPSA.FACVENTA F
                                 INNER JOIN FACTURACIONBEPSA.FACVENTADET FVD ON F.FACVENTAID = FVD.FACVENTAID
                                 INNER JOIN FACTURACIONBEPSA.CLIENTE C ON C.TIPOCLIENTEID = F.TIPOCLIENTEID AND C.CLIENTEID = F.CLIENTEID
                                 LEFT JOIN FACTURACIONBEPSA.FACVENTARTSET FVS ON F.FACVENTAID = FVS.FACVENTAID
                            AND FVS.FACVENTARTSETPROCESAMIENTOCORR = TRUE
                        WHERE F.EJERCICIOID = 2025
                          AND F.PERIODOID = 01
                          AND F.TIPOCOMPROBANTEID = 'FAECA'
                          AND F.TIMBRADOID = '16410772'
                          AND F.ESTABLECIMIENTOID = '001'
                          ...
                          AND F.FACVENTACONDICION = 'CNTDO'
                          AND C.TIPOCLIENTEID = 'Comercio'
                          AND C.CLIENTETIPOCONTRIB = 'NCONT' -- nueva condición para discriminar por tipo contribuyente
                          AND F.STATUSID IN ('DTE_APROBADO')
                          AND FVD.SERVPRESTID IN (SELECT *
                                                  FROM FACTURACIONBEPSA.FN_OBTENER_PARMVALUE('desc_serv_prest', 'C'))
                          AND (F.FACVENTARETENSETMONTO + F.FACVENTARETENSETIVA) > 0);

select count(*) from asientos.facturacionbepsa.CLIENTE C where C.CLIENTETIPOCONTRIB = 'NCONT'
and C.TIPOCLIENTEID='Comercio';

--ver
SELECT *
FROM FACTURACIONBEPSA.CLIENTE C
WHERE C.CLIENTETIPOPERSONA = 'PERFI'
  AND (C.ULTIMACONSULTARUC IS NULL OR C.ULTIMACONSULTARUC < DATE_TRUNC('month', CURRENT_DATE))
  AND TIPOCLIENTEID != 'Funcionario'
LIMIT 30000;

--ver facvventa id que existen ya en facventartset
SELECT *
FROM FACTURACIONBEPSA.FACVENTARTSET F
         INNER JOIN FACTURACIONBEPSA.FACVENTA F2 ON F2.FACVENTAID = F.FACVENTAID
         INNER JOIN FACTURACIONBEPSA.CLIENTE C ON F2.TIPOCLIENTEID = C.TIPOCLIENTEID AND F2.CLIENTEID = C.CLIENTEID
WHERE C.CLIENTETIPOCONTRIB = 'NCONT'
  AND C.TIPOCLIENTEID = 'Comercio'
  AND F2.PERIODOID = 01
  AND F2.EJERCICIOID = 2025
  AND F2.TARGETFACTID = 'FactComercios';
--bk y se elimina para vovler a procesar. Una vez hecho se descomenta de la cabecera para ver si carga.
CREATE TABLE PUBLIC.BKRETENCION AS (SELECT * FROM FACTURACIONBEPSA.FACVENTARTSET F WHERE FACVENTAID
IN ('906215', '906344', '906567', '906665', '906737', '907049', '907889', '908410', '908645', '908885',
'909393', '909825', '909830', '909969', '910398', '910467', '911162'));
--fea63f18-33cc-4e38-b787-47f1779abd95
--parametro para envio manual
select * from facturacionbepsa.parametros pp
where pp.parmid='setRetencionesEventos' and pp.parmserial ='procesarManual';
---para concat facventaid  setRetencionesEventos
select * from facturacionbepsa.parametros p
where p.parmid ='setRetencionesEventos' and p.parmserial ='cambiarValorFacVentaID';--parmvalue a NO
select * from facturacionbepsa.parametros p
...
SELECT facventatotalcomprobml, F.* FROM asientos.facturacionbepsa.FACVENTA F;
...
WHERE STATUSID ='DE_ACTIVO'
  AND CAST(EXTRACT(EPOCH FROM (TIMEZONE('America/Asuncion'::TEXT, NOW()) - FACVENTASTATUSLASTFH)) / 60 AS INTEGER) > :tiempoMinutos
ORDER BY facventanumero;

SELECT CAST(EXTRACT(EPOCH FROM (TIMEZONE('America/Asuncion'::TEXT, NOW()) - FACVENTASTATUSLASTFH)) / 60 AS INTEGER)
FROM asientos.facturacionbepsa.FACVENTA F WHERE facventaid IN (995846, 995847);

SELECT * FROM asientos.facturacionbepsa.PARAMETROS P where P.PARMSERIAL='tiempoMinutos';

SELECT * FROM TRUSTED_ZONE.GXFINDTA_TCLMOV T WHERE T.MOVRRNBEP IN ('507159768235','506456977799',
'508566140070');
---
UPDATE trusted_zone.gxfindta_tclmov SET movimco = 0.00::numeric(17,2), movivco = 0.00::numeric(17,2), movcoco = 0.0000::numeric(19,4), movrent = 0.00::numeric(17,2), movivren = 0.00::numeric(17,2) WHERE movidmov = 193308610::bigint AND sercodi LIKE 'COMPRA' ESCAPE '#' AND precodi LIKE 'COMP' ESCAPE '#' AND movftrx LIKE '20250305' ESCAPE '#' AND movhtrx LIKE '17:26:11' ESCAPE '#' AND movesta LIKE 'A' ESCAPE '#' AND movcodis LIKE 'POS' ESCAPE '#' AND movfpgo LIKE 'TA' ESCAPE '#' AND movfpro LIKE '20250305' ESCAPE '#' AND movfcre LIKE '20250306' ESCAPE '#' AND movfsip LIKE '20250305' ESCAPE '#' AND movcoau LIKE '380182' ESCAPE '#' AND movidtr LIKE 'PX00587832' ESCAPE '#' AND movtpta LIKE 'C' ESCAPE '#' AND movntar LIKE '422410XXXXXX2659' ESCAPE '#' AND movcoam LIKE 'L' ESCAPE '#' AND movbepsa LIKE 'ISA' ESCAPE '#' AND movdesam LIKE 'Local' ESCAPE '#' AND movcodem = 102001::integer AND movdesem LIKE 'CONTINENTAL' ESCAPE '#' AND movcodad = 9000::smallint AND movdesad LIKE 'BEPSA' ESCAPE '#' AND movcodpr = 9000::smallint AND movdespr LIKE 'BEPSA' ESCAPE '#' AND movcdbce = 100204::integer AND movdsbce LIKE 'BNF' ESCAPE '#' AND movtpbce LIKE 'CC' ESCAPE '#' AND movctbce LIKE '8191198' ESCAPE '#' AND movimpo = 612297.00::numeric(17,2) AND movimco = 18369.00::numeric(17,2) AND movivco = 1837.00::numeric(17,2) AND movcoco = 3.0000::numeric(19,4) AND movcoma = 0.0000::numeric(19,4) AND movrete LIKE 'Todos' ESCAPE '#' AND movvere LIKE '00000000' ESCAPE '#' AND movrent = 6123.00::numeric(17,2) AND movivren = 5566.00::numeric(17,2) AND movneto = 580402.00::numeric(17,2) AND movpgcom IS NULL AND movespg IS NULL AND movmcc LIKE '5912' ESCAPE '#' AND movcomer LIKE '1003886' ESCAPE '#' AND movcdbcc = 102004::integer AND movdeno LIKE 'FARMACENTER FORTIS CDE' ESCAPE '#' AND movrazo LIKE 'CADENA FARMACENTER S.A.' ESCAPE '#' AND movruc LIKE '80019551-5' ESCAPE '#' AND movcdlo LIKE '020' ESCAPE '#' AND movdelo LIKE 'CIUDAD DEL ESTE' ESCAPE '#' AND movcddto LIKE '010' ESCAPE '#' AND movdedto LIKE 'ALTO PARANA' ESCAPE '#' AND movcdzon LIKE '66' ESCAPE '#' AND movdezon LIKE 'ZONA 66' ESCAPE '#' AND movcate IS NULL AND movcdtpc LIKE '1' ESCAPE '#' AND movdetpc LIKE 'Acred. en Cta' ESCAPE '#' AND movpeac LIKE '2' ESCAPE '#' AND movdepe LIKE '24 HS' ESCAPE '#' AND movcdrep LIKE '3C' ESCAPE '#' AND movderep LIKE 'JESSICA BENITEZ' ESCAPE '#' AND movdsbcc LIKE 'CONTINENTAL' ESCAPE '#' AND movtpcnx LIKE 'DIR' ESCAPE '#' AND movtpbcc LIKE 'CC' ESCAPE '#' AND movctbcc LIKE '0108824731' ESCAPE '#' AND movadmi = 900002::integer AND movprop = 900003::integer AND movmarc LIKE 'VSA' ESCAPE '#' AND movprod LIKE 'CLA' ESCAPE '#' AND movafin = 37::smallint AND movpgon LIKE 'B' ESCAPE '#' AND movpepg LIKE '2' ESCAPE '#' AND movespc IS NULL AND movopor = 0::integer AND movopde = 0::integer AND movteor IS NULL AND movtede IS NULL AND movidlt LIKE '250640011003886' ESCAPE '#' AND movfeac LIKE '20250306' ESCAPE '#' AND movusac LIKE 'QUSER' ESCAPE '#' AND movbcab = 1020::smallint AND movrrnbep LIKE '506456977799' ESCAPE '#' AND movpaisori = 600::smallint AND movmonori = 600::smallint AND movdspais LIKE 'PARAGUAY' ESCAPE '#' AND movdsmoned LIKE 'GUARANI' ESCAPE '#' AND movncbcab LIKE 'CONTINENTAL' ESCAPE '#' AND movcodswd LIKE 'BNFAPYPA' ESCAPE '#' AND movcodswc LIKE 'BCNAPYPA' ESCAPE '#' AND movmonsig LIKE 'PYG' ESCAPE '#' AND movidtpdoc LIKE 'RUC' ESCAPE '#' AND movnrodoc LIKE '80019551-5' ESCAPE '#' AND movcodcli = 11104::integer AND movcodsuc = 223::integer AND movfecliq LIKE '20250305' ESCAPE '#' AND movtpopag LIKE 'CUO' ESCAPE '#' AND movcancuo = 2::smallint AND movtimbra IS NULL AND movfacexp IS NULL AND movfacsuc IS NULL AND movfacnro = 0::integer AND movtpocom LIKE 'P' ESCAPE '#' AND movvalcom = 3.00::numeric(17,2) AND movica LIKE '422410225' ESCAPE '#' AND movemibco = 1020::integer AND movemctbc IS NULL AND movadmbco = 9000::integer AND movadctbc IS NULL AND movtartok IS NULL AND movdscadm LIKE 'BEPSA' ESCAPE '#' AND movdscprop LIKE 'BEPSA' ESCAPE '#' AND movsdrent = 0.00::numeric(17,2) AND movsdivre = 0.00::numeric(17,2) AND movcodpra = 0::bigint AND movtpocaj IS NULL AND movimpai = 600::numeric(3) AND movimmda = 600::numeric(3) AND movmdacot = 0.00::numeric(15,2) AND movimtrx = 0.00::numeric(15,2) AND movmarcot = 0.00::numeric(15,2) AND movtsad = 0.00::numeric(15,2) AND movinad = 0.00::numeric(15,2) AND movivad = 0.00::numeric(15,2);
UPDATE trusted_zone.gxfindta_tclmov SET movimco = 0.00::numeric(17,2), movivco = 0.00::numeric(17,2), movcoco = 0.0000::numeric(19,4), movrent = 0.00::numeric(17,2), movivren = 0.00::numeric(17,2) WHERE movidmov = 197274584::bigint AND sercodi LIKE 'COMPRA' ESCAPE '#' AND precodi LIKE 'COMP' ESCAPE '#' AND movftrx LIKE '20250326' ESCAPE '#' AND movhtrx LIKE '17:46:41' ESCAPE '#' AND movesta LIKE 'A' ESCAPE '#' AND movcodis LIKE 'POS' ESCAPE '#' AND movfpgo LIKE 'TA' ESCAPE '#' AND movfpro LIKE '20250326' ESCAPE '#' AND movfcre LIKE '20250327' ESCAPE '#' AND movfsip LIKE '20250326' ESCAPE '#' AND movcoau LIKE '090934' ESCAPE '#' AND movidtr LIKE 'PX12024446' ESCAPE '#' AND movtpta LIKE 'C' ESCAPE '#' AND movntar LIKE '459430XXXXXX9652' ESCAPE '#' AND movcoam LIKE 'L' ESCAPE '#' AND movbepsa LIKE 'ISA' ESCAPE '#' AND movdesam LIKE 'Local' ESCAPE '#' AND movcodem = 102001::integer AND movdesem LIKE 'CONTINENTAL' ESCAPE '#' AND movcodad = 9000::smallint AND movdesad LIKE 'BEPSA' ESCAPE '#' AND movcodpr = 9000::smallint AND movdespr LIKE 'BEPSA' ESCAPE '#' AND movcdbce = 100204::integer AND movdsbce LIKE 'BNF' ESCAPE '#' AND movtpbce LIKE 'CC' ESCAPE '#' AND movctbce LIKE '8191198' ESCAPE '#' AND movimpo = 1000000.00::numeric(17,2) AND movimco = 30000.00::numeric(17,2) AND movivco = 3000.00::numeric(17,2) AND movcoco = 3.0000::numeric(19,4) AND movcoma = 0.0000::numeric(19,4) AND movrete LIKE 'Todos' ESCAPE '#' AND movvere LIKE '00000000' ESCAPE '#' AND movrent = 10000.00::numeric(17,2) AND movivren = 9090.00::numeric(17,2) AND movneto = 947910.00::numeric(17,2) AND movpgcom IS NULL AND movespg IS NULL AND movmcc LIKE '5047' ESCAPE '#' AND movcomer LIKE '2700397' ESCAPE '#' AND movcdbcc = 102004::integer AND movdeno LIKE 'OPTICA VALEMAR' ESCAPE '#' AND movrazo LIKE 'MULTILAB PY S.A.' ESCAPE '#' AND movruc LIKE '80090939-9' ESCAPE '#' AND movcdlo LIKE '002' ESCAPE '#' AND movdelo LIKE 'SAN LORENZO' ESCAPE '#' AND movcddto LIKE '011' ESCAPE '#' AND movdedto LIKE 'CENTRAL' ESCAPE '#' AND movcdzon LIKE '60' ESCAPE '#' AND movdezon LIKE 'ZONA 60' ESCAPE '#' AND movcate IS NULL AND movcdtpc LIKE '1' ESCAPE '#' AND movdetpc LIKE 'Acred. en Cta' ESCAPE '#' AND movpeac LIKE '2' ESCAPE '#' AND movdepe LIKE '24 HS' ESCAPE '#' AND movcdrep LIKE '7J' ESCAPE '#' AND movderep LIKE 'CESAR OJEDA' ESCAPE '#' AND movdsbcc LIKE 'CONTINENTAL' ESCAPE '#' AND movtpcnx LIKE 'DIR' ESCAPE '#' AND movtpbcc LIKE 'CC' ESCAPE '#' AND movctbcc LIKE '2907166513' ESCAPE '#' AND movadmi = 900002::integer AND movprop = 900003::integer AND movmarc LIKE 'VSA' ESCAPE '#' AND movprod LIKE 'ORO' ESCAPE '#' AND movafin = 51::smallint AND movpgon LIKE 'B' ESCAPE '#' AND movpepg LIKE '2' ESCAPE '#' AND movespc IS NULL AND movopor = 0::integer AND movopde = 0::integer AND movteor IS NULL AND movtede IS NULL AND movidlt LIKE '250850012700397' ESCAPE '#' AND movfeac LIKE '20250327' ESCAPE '#' AND movusac LIKE 'QUSER' ESCAPE '#' AND movbcab = 1020::smallint AND movrrnbep LIKE '508566140070' ESCAPE '#' AND movpaisori = 600::smallint AND movmonori = 600::smallint AND movdspais LIKE 'PARAGUAY' ESCAPE '#' AND movdsmoned LIKE 'GUARANI' ESCAPE '#' AND movncbcab LIKE 'CONTINENTAL' ESCAPE '#' AND movcodswd LIKE 'BNFAPYPA' ESCAPE '#' AND movcodswc LIKE 'BCNAPYPA' ESCAPE '#' AND movmonsig LIKE 'PYG' ESCAPE '#' AND movidtpdoc LIKE 'RUC' ESCAPE '#' AND movnrodoc LIKE '80090939-9' ESCAPE '#' AND movcodcli = 19655::integer AND movcodsuc = 1::integer AND movfecliq LIKE '20250326' ESCAPE '#' AND movtpopag LIKE 'CUO' ESCAPE '#' AND movcancuo = 7::smallint AND movtimbra IS NULL AND movfacexp IS NULL AND movfacsuc IS NULL AND movfacnro = 0::integer AND movtpocom LIKE 'P' ESCAPE '#' AND movvalcom = 3.00::numeric(17,2) AND movica LIKE '45943000' ESCAPE '#' AND movemibco = 1020::integer AND movemctbc IS NULL AND movadmbco = 9000::integer AND movadctbc IS NULL AND movtartok IS NULL AND movdscadm LIKE 'BEPSA' ESCAPE '#' AND movdscprop LIKE 'BEPSA' ESCAPE '#' AND movsdrent = 0.00::numeric(17,2) AND movsdivre = 0.00::numeric(17,2) AND movcodpra = 0::bigint AND movtpocaj IS NULL AND movimpai = 600::numeric(3) AND movimmda = 600::numeric(3) AND movmdacot = 0.00::numeric(15,2) AND movimtrx = 0.00::numeric(15,2) AND movmarcot = 0.00::numeric(15,2) AND movtsad = 0.00::numeric(15,2) AND movinad = 0.00::numeric(15,2) AND movivad = 0.00::numeric(15,2);
UPDATE trusted_zone.gxfindta_tclmov SET movimco = 0.00::numeric(17,2), movivco = 0.00::numeric(17,2), movcoco = 0.0000::numeric(19,4), movcoma = 0.0000::numeric(19,4), movrent = 0.00::numeric(17,2), movivren = 0.00::numeric(17,2) WHERE movidmov = 194621443::bigint AND sercodi LIKE 'COMPRA' ESCAPE '#' AND precodi LIKE 'COQR' ESCAPE '#' AND movftrx LIKE '20250312' ESCAPE '#' AND movhtrx LIKE '20:07:22' ESCAPE '#' AND movesta LIKE 'A' ESCAPE '#' AND movcodis LIKE 'POS' ESCAPE '#' AND movfpgo LIKE 'QR' ESCAPE '#' AND movfpro LIKE '20250312' ESCAPE '#' AND movfcre LIKE '20250313' ESCAPE '#' AND movfsip LIKE '20250312' ESCAPE '#' AND movcoau LIKE '192844' ESCAPE '#' AND movidtr LIKE 'PX0003364692' ESCAPE '#' AND movtpta LIKE 'C' ESCAPE '#' AND movntar LIKE '545288XXXXXX7584' ESCAPE '#' AND movcoam LIKE 'R' ESCAPE '#' AND movbepsa LIKE 'ADQ' ESCAPE '#' AND movdesam LIKE 'Intra Country' ESCAPE '#' AND movcodem = 900401::integer AND movdesem LIKE 'BANCARD' ESCAPE '#' AND movcodad = 9000::smallint AND movdesad LIKE 'BEPSA' ESCAPE '#' AND movcodpr = 9004::smallint AND movdespr LIKE 'BANCARD' ESCAPE '#' AND movcdbce = 900004::integer AND movdsbce LIKE 'BEPSA' ESCAPE '#' AND movtpbce LIKE 'CC' ESCAPE '#' AND movctbce LIKE '0000000000123548' ESCAPE '#' AND movimpo = 334295.00::numeric(17,2) AND movimco = 10029.00::numeric(17,2) AND movivco = 1003.00::numeric(17,2) AND movcoco = 3.0000::numeric(19,4) AND movcoma = 2.6400::numeric(19,4) AND movrete LIKE 'Todos' ESCAPE '#' AND movvere LIKE '00000000' ESCAPE '#' AND movrent = 3343.00::numeric(17,2) AND movivren = 3039.00::numeric(17,2) AND movneto = 316881.00::numeric(17,2) AND movpgcom IS NULL AND movespg IS NULL AND movmcc LIKE '5912' ESCAPE '#' AND movcomer LIKE '1002922' ESCAPE '#' AND movcdbcc = 102004::integer AND movdeno LIKE 'FARMACENTER LAGUNA GRANDE 2' ESCAPE '#' AND movrazo LIKE 'CADENA FARMACENTER S.A.' ESCAPE '#' AND movruc LIKE '80019551-5' ESCAPE '#' AND movcdlo LIKE '005' ESCAPE '#' AND movdelo LIKE 'FERNANDO DE LA MORA' ESCAPE '#' AND movcddto LIKE '011' ESCAPE '#' AND movdedto LIKE 'CENTRAL' ESCAPE '#' AND movcdzon LIKE '34' ESCAPE '#' AND movdezon LIKE 'ZONA 34' ESCAPE '#' AND movcate IS NULL AND movcdtpc LIKE '1' ESCAPE '#' AND movdetpc LIKE 'Acred. en Cta' ESCAPE '#' AND movpeac LIKE '2' ESCAPE '#' AND movdepe LIKE '24 HS' ESCAPE '#' AND movcdrep LIKE '3C' ESCAPE '#' AND movderep LIKE 'JESSICA BENITEZ' ESCAPE '#' AND movdsbcc LIKE 'CONTINENTAL' ESCAPE '#' AND movtpcnx LIKE 'DIR' ESCAPE '#' AND movtpbcc LIKE 'CC' ESCAPE '#' AND movctbcc LIKE '0108824731' ESCAPE '#' AND movadmi = 900002::integer AND movprop = 900003::integer AND movmarc LIKE 'MST' ESCAPE '#' AND movprod LIKE 'CLA' ESCAPE '#' AND movafin = 0::smallint AND movpgon LIKE 'B' ESCAPE '#' AND movpepg LIKE '2' ESCAPE '#' AND movespc IS NULL AND movopor = 0::integer AND movopde = 0::integer AND movteor IS NULL AND movtede IS NULL AND movidlt LIKE '250710011002922' ESCAPE '#' AND movfeac LIKE '20250313' ESCAPE '#' AND movusac LIKE 'QUSER' ESCAPE '#' AND movbcab = 1020::smallint AND movrrnbep LIKE '507159768235' ESCAPE '#' AND movpaisori = 600::smallint AND movmonori = 600::smallint AND movdspais LIKE 'PARAGUAY' ESCAPE '#' AND movdsmoned LIKE 'GUARANI' ESCAPE '#' AND movncbcab LIKE 'CONTINENTAL' ESCAPE '#' AND movcodswd LIKE 'BEPSPYPA' ESCAPE '#' AND movcodswc LIKE 'BCNAPYPA' ESCAPE '#' AND movmonsig LIKE 'PYG' ESCAPE '#' AND movidtpdoc LIKE 'RUC' ESCAPE '#' AND movnrodoc LIKE '80019551-5' ESCAPE '#' AND movcodcli = 11104::integer AND movcodsuc = 197::integer AND movfecliq LIKE '20250312' ESCAPE '#' AND movtpopag LIKE 'CON' ESCAPE '#' AND movcancuo = 1::smallint AND movtimbra IS NULL AND movfacexp IS NULL AND movfacsuc IS NULL AND movfacnro = 0::integer AND movtpocom LIKE 'P' ESCAPE '#' AND movvalcom = 3.00::numeric(17,2) AND movica LIKE '545288' ESCAPE '#' AND movemibco = 9004::integer AND movemctbc IS NULL AND movadmbco = 9000::integer AND movadctbc IS NULL AND movtartok IS NULL AND movdscadm LIKE 'BEPSA' ESCAPE '#' AND movdscprop LIKE 'BEPSA' ESCAPE '#' AND movsdrent = 0.00::numeric(17,2) AND movsdivre = 0.00::numeric(17,2) AND movcodpra = 0::bigint AND movtpocaj IS NULL AND movimpai = 600::numeric(3) AND movimmda = 600::numeric(3) AND movmdacot = 0.00::numeric(15,2) AND movimtrx = 0.00::numeric(15,2) AND movmarcot = 0.00::numeric(15,2) AND movtsad = 0.00::numeric(15,2) AND movinad = 0.00::numeric(15,2) AND movivad = 0.00::numeric(15,2);
