--brelac
SELECT DISTINCT  WHPNAM,WHTEXT FROM WRKADOLFO.BRELACPROD WHERE WHFNAM LIKE '%DEBCREDIFZ%';
--------------------------------------------ver log as
...
SELECT * FROM TABLE(QSYS2.HISTORY_LOG_INFO('2024-12-19-23.40.00.000000', '2024-12-20-00.40.00.000000')) A
WHERE --FROM_JOB_NAME='QRWTSRVR'-- AND fROM_USER = 'QUSER'; --AND FROM_JOB_NAME='VISAADQJ';
FROM_JOB LIKE '396175/USRRGNCON/ARGPUNTOS'; --AND MESSAGE_ID='CPF1164';
...
FROM TABLE (QSYS2.OBJECT_STATISTICS('GXTRJPGM','*PGM')) X
WHERE OBJNAME LIKE '%PMIG04%';
...
DELETE FROM ATMCENTRE.FMTDEBATMC f WHERE f.DEBUGTIMST < TIMESTAMP ( CURRENT DATE - 15 DAYS , TIME ('00:00:00'));
DELETE FROM GXFINDTA.TPRLOG t WHERE t.LOGID <= 1009451321;
SELECT * FROM GXAUTDTA.TAUA00; --CLRPFM
SELECT * FROM gxautdta.TAUCNX; --CAMBIAR NUEVAMENTE A A
DELETE FROM HSTTRNPROC.TRNPRCDEB WHERE PRCDEBTIM < TIMESTAMP(CURRENT DATE - 15 DAYS,TIME('00:00:00')) ;
SELECT COUNT(*) FROM HSTTRNPROC.TRNPRCDEB T;
SELECT COUNT(*) FROM ATMCENTRE.FMTDEBATMC F;